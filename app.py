...
@app.route('/v2/admin/immune/unquarantine-health-snapshots', methods=['POST'])
@require_admin
def admin_unquarantine_health_snapshots():
    """Clean up the existing quarantine queue of health_snapshot entries.

    Companion to the DUPLICATE_EMBEDDING content-type exclusion in patrol_
    duplicate_embeddings. That fix stops new health_snapshot rows from
    being quarantined; this endpoint reverses the ones already sitting in
    the queue from earlier misreads.

    Safe by construction:
      - only targets rows where content_type = 'health_snapshot'
      - only unquarantines rows currently quarantined
    ...
    try:
        cur.execute("""
            SELECT COUNT(*) AS n
            FROM blobs
            WHERE content_type = 'health_snapshot'
              AND COALESCE(quarantined, FALSE) = TRUE
        """)
        ...
        cur.execute("""
            SELECT COUNT(*) AS n
            FROM blobs
            WHERE COALESCE(quarantined, FALSE) = TRUE
        """)
        remaining_quarantine = cur.fetchone()['n']

        return jsonify({
            'dry_run': dry_run,
            'candidate_count': candidate_count,
            'updated': updated,
            'remaining_quarantine_count': remaining_quarantine,
        })
    except Exception as e:
        db.rollback()
        return jsonify({'error': str(e)}), 500
